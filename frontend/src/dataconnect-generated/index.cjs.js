...
const hapusUserRef = (dcOrVars, vars) => {
  const { dc: dcInstance, vars: inputVars} = validateArgs(connectorConfig, dcOrVars, vars, true);
  dcInstance._useGeneratedSdk();
  return mutationRef(dcInstance, 'HapusUser', inputVars);
}
hapusUserRef.operationName = 'HapusUser';
exports.hapusUserRef = hapusUserRef;

exports.hapusUser = function hapusUser(dcOrVars, vars) {
  const { dc: dcInstance, vars: inputVars } = validateArgs(connectorConfig, dcOrVars, vars, true);
  return executeMutation(hapusUserRef(dcInstance, inputVars));
}
;

const ubahRoleUserRef = (dcOrVars, vars) => {
  const { dc: dcInstance, vars: inputVars} = validateArgs(connectorConfig, dcOrVars, vars, true);
  dcInstance._useGeneratedSdk();
  return mutationRef(dcInstance, 'ubahRoleUser', inputVars);
}
ubahRoleUserRef.operationName = 'ubahRoleUser';
exports.ubahRoleUserRef = ubahRoleUserRef;

exports.ubahRoleUser = function ubahRoleUser(dcOrVars, vars) {
  const { dc: dcInstance, vars: inputVars } = validateArgs(connectorConfig, dcOrVars, vars, true);
